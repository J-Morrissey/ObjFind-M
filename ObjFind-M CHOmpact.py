...
CONFIG = {
    # --- Inputs ---
    "model_path": "CHOmpact.json",
    "exchange_file": "CHOmpact Metabolite Exchange Rates.xlsx",  # rows: exchange rxn IDs, cols: experiments
    "intracellular_file": "Intracellular Reactions All.xlsx",
    "mapping_file": "CHOmpact Mapping ObjFind-M.xlsx",
    "experiment": "SVM1",

    # --- Objective hooks (OPTIONAL) ---
    # If you want extra (v - measured)^2 terms for specific reactions, add them here
    # with numeric targets. (Exchanges are already fixed to measurements below.)
    "exometabolite_targets": {
        # "F142": <measured_biomass_flux_for_SVM1>,
        # "F143": <measured_igg_flux_for_SVM1>,
    },

    # --- Model overrides (OPTIONAL) ---
    # These mirror your manual bounds:
    "force_bounds": {
        "F144": (0, 0),    # alternative ATP demand OFF
        "F142": (0, 10),   # biomass
        "F143": (0, 10),   # IgG
        "F104": (0, 10),   # ATP maintenance
        "F77":  (0, 0),    # NADP_tot --> NAD_tot
        "F103": (0, 0),    # NADH_cyt --> NAD_tot
    },

    # Set exchange reactions exactly to measured fluxes from the sheet (incl. *_reverse if present)
    ...
    # Output
    "output_excel": "objfindm Results.xlsx",
}
